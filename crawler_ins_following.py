...
def crawler_following(uid):
	...
	# print data
	if data['status'] == 'ok':
		count = data['data']['user']['edge_follow']['count']
		followings = extract_edges(data)
		while data['data']['user']['edge_follow']['page_info']['has_next_page']:
			...
			data = data.json()
			if data['status'] == 'ok':
				followings.extend(extract_edges(data))
			else:
				break
	else:
		if data['message'] == 'rate limited':
			...
			time.sleep(120)
			return crawler_following(uid)
	return {'count': count, 'nodes': followings}
...
def extract_edges(data):
	...
	if data['status'] == 'ok':
		for edge in data['data']['user']['edge_follow']['edges']:
			following = {}
			following['id'] = edge['node']['id']
			following['username'] = edge['node']['username']
			following['full_name'] = edge['node']['full_name']
			following['is_verified'] = edge['node']['is_verified']
...
def load_crawl_list():
	...
	if os.path.exists("user_info_ins_following_%s.txt" % machine_id):
		infile = codecs.open("user_info_ins_following_%s.txt" % machine_id, "r")
		...
		infile.close()
		output.close()
		os.remove("user_info_ins_following_%s.txt" % machine_id)
		os.rename("temp.txt", "user_info_ins_following_%s.txt" % machine_id)
		print xx
...
if __name__ == '__main__':
	...
	time.sleep(0.1)
	load_crawl_list()
	fout = codecs.open("user_info_ins_following_%s.txt" % machine_id, 'a')
	num = 0
	num_done = 0
	num_err = 0
	for (aboutme_id, uid) in id_list:
		...
		if aboutme_id not in done_set:
			...
			print num, ': ', uid
			try:
				info['followings'] = crawler_following(uid)
